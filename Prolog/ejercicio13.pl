...
% Codifica directamente una lista, agrupando elementos consecutivos iguales.
encode_direct([], []).
encode_direct([X|Xs], [[N,X]|R]) :- count(X, [X|Xs], N, Rest), encode_direct(Rest, R).
...
ejercicio13 :-
    Input = [a, a, b, b, c, c, c, d, d, d, d],  % Cambia la lista para probar diferentes casos
    encode_direct(Input, Encoded),
    format('La lista codificada de ~w es ~w.~n', [Input, Encoded]).

% Ejecutar el predicado ejercicio13 al cargar el archivo.
:- ejercicio13.
